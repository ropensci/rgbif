...
Large datasets of millions of lat-lon points can be difficult to plot using `ggplot2` or base graphics. In these cases, using the [GBIF maps API](https://www.gbif.org/developer/maps) from with `rgbif::map_fetch()` can be a good option. 

However, if you would like to make a complex graphic with different colors, shapes, and legends, using `ggplot2::geom_sf()` is your only option. Below I will give examples using both methods.  
...
![](C:/Users/ftw712/Desktop/packages/rgbif/man/figures/dot_map.png){width=50%}
...
![](C:/Users/ftw712/Desktop/packages/rgbif/man/figures/gbif-light-scaled-circles-birds.png){width=50%}
...
![](C:/Users/ftw712/Desktop/packages/rgbif/man/figures/hexpertile.png){width=50%}
...
![](C:/Users/ftw712/Desktop/packages/rgbif/man/figures/artic-peguins-glacier-point-gbif-dark.png)
...
![](C:/Users/ftw712/Desktop/packages/rgbif/man/figures/AU-zoom.png){width=50%}
...
Below is a table of some areas to give you an idea of how `z`,`x`,`y` it are working. 

| Location    | Parameters
| ----------- | -----------------------------------------|
| Europe      | `map_fetch(z=3,x=7:9,y=0:2)`             | 
| Africa      | `map_fetch(z=3,x=7:10,y=2:5)`            | 
| Hawaii      | `map_fetch(z=6,x=6:9,y=23:25)`           |
| South Africa| `map_fetch(z=5,x=34:38,y=19:22)`         |
| Ukraine     | `map_fetch(z=6,x=70:78,y=13:16)`         |
| Iceland     | `map_fetch(z=6,x=55:59,y=8:9)`           | 
| Capri Is.   | `map_fetch(z=12,x=4419:4420,y=1124:1125)`|
...
![](C:/Users/ftw712/Desktop/packages/rgbif/man/figures/adhoc-iucn.png){width=50%}
...
The table below gives some examples of maps in different styles. 

| code                                  | comment
| --------------------------------------| ---------------|
| `map_fetch(recordedBy="John Waller")` | `adhoc` is needed here  because `recordedBy` isn't one of the named paramters. `map_fetch()` automatically detects this and switches source to "adhoc". |
| `map_fetch(z=1,source="adhoc",style="green.poly",squareSize=64,bin="square",network_key="2b7c7b4f-4d4f-40d3-94de-c28b6fa054a6")` | Occurrences in the OBIS network. Note that `squareSize` only works with `bin="square"`. |
| `map_fetch(z=4,x=6:7,y=4:5,gadm_gid="USA.44_1",style="blue.marker")` | Map of Texas using the `gadm` filter. |
| `map_fetch(x=1:5)` | `map_fetch()` is generally forgiving and will give you back at least some map with warnings or blank images when the parameters don't work. |
| `map_fetch(taxonKey=212, basisOfRecord="PRESERVED_SPECIMEN", year=2000,style="classic-noborder.poly")` | All specimen bird records from the year 2000. | 
| `map_fetch(distanceFromCentroidInMeters=0,base_style="osm-bright")` | Map of all country centroid locations. |
| `map_fetch(source="adhoc",project_id="BID-AF2015-0134-REG",style="green2.poly",hexPerTile=50)` | In general, adhoc maps are harder to make look nice, but usually picking a non-point style and tuning `hexPerTile` and perhaps also `squareSize`, will make a nice map. |
...
# occ_download(pred_default(),pred(taxonKey,"7412043"),format="SIMPLE_CSV")
