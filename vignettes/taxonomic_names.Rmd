...
When two or more names exist in the taxonomy that have the **same name** but **different authorship** (homotypic synonyms), supplying just the binomial name will result in `matchType : "HIGHERRANK"`. 

For example, the binomial name "Glocianus punctiger" has two entries in the taxonomy. Using the `verbose=TRUE` will return both names. 
...
There are several functions for finding taxonomic information. Typically, the function you want to use is `name_backbone` or `name_backbone_checklist`.

**⚠️ DEPRECATION NOTICE:** The following functions (`name_suggest`, `name_lookup`, `name_usage`) only work with the **out-of-date GBIF Backbone Taxonomy** and do **not** support the `checklistKey` parameter for COL (Catalogue of Life) Extended Release. These functions may be deprecated in a future version. Consider using the `rcol` package instead, which provides direct access to COL Extended Release via `col_search()`, `col_suggest()`, and `col_usage()` functions.

`name_suggest` can be useful for looking up subspecies or partial names, but only searches the GBIF Backbone Taxonomy. It is the same service that lets [gbif.org](https://www.gbif.org) guess which name you are typing in the [occurrence search](https://www.gbif.org/occurrence/search).

```r
name_suggest("Calopteryx splendens")
```

`name_lookup` can be sometimes useful for seeing what is available in other checklists, but only searches the GBIF Backbone Taxonomy.

```r
name_lookup("Calopteryx splendens")$data
```

`name_usage` is a catch all function that does a lot, but only works with the GBIF Backbone Taxonomy. `?name_usage` for more examples. 
