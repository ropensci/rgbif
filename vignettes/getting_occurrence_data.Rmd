...
The extension tables available for download are provided using this [API call](https://api.gbif.org/v1/occurrence/download/describe/dwca)
, under **verbatimExtensions**. They can be requested by adding a `verbatim_extensions()` expression to the `occ_download()` request. The format has to be "DWCA" for `verbatim_extensions()` to work. 
