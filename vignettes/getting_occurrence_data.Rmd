...
For users interested in aggregating occurrence data or selecting specific columns, see the [GBIF SQL Downloads](https://docs.ropensci.org/rgbif/articles/gbif_sql_downloads.html) vignette.
...
**Note:** By default, **rgbif** now uses the COL (Catalogue of Life) Extended Release taxonomy, which returns alpha-numeric taxon keys (e.g., "Q2M4"). The examples below use these alpha-numeric keys.

| function         | description                           | example                                                         |
|:------------------------|:------------------------|:---------------------|
| `pred()`         | key is equal to value                 | `pred("taxonKey","Q2M4")`                                       |
...
| `pred_not()`     | key is not value                      | `pred_not("taxonKey","Q2M4")`                                   |
...
| `pred_and()`     | a logical and of predicate functions  | `pred_and(pred_lte("elevation",5000),pred("taxonKey","Q2M4"))`  |
| `pred_or()`      | a logical or of predicate functions   | `pred_or(pred_gt("elevation", 1000), pred_isnull("elevation"))` |
| `pred_in()`      | values are in the column              | `pred_in("taxonKey",c("Q2M4","9WLSS","Q2N2"))`                  |

**Note on license values:** When filtering by license, use underscores instead of spaces (e.g., `"CC_BY_4_0"` not `"CC BY 4.0"`). Valid license values include `"CC_BY_4_0"`, `"CC_BY_NC_4_0"`, `"CC0_1_0"`.
...
The following will download all occurrences of *Calopteryx splendens*. You can use `name_backbone("Calopteryx splendens")` to find the taxonKey (usageKey), which returns "Q2M4" with COL XR.
...
If you want to download only records with specific licenses, use `pred()` or `pred_in()` with the license key. **Important:** Always use underscores in license values, not spaces.
...
**rgbif** now uses the **Catalogue of Life Extended Release (COL XR)** as the default taxonomy, which returns alpha-numeric taxon keys. To use the original **GBIF Backbone Taxonomy** instead, set `checklistKey = NULL`.

You need to use the corresponding taxonKey from the chosen taxonomy. You can find these keys using `name_backbone()` with or without the `checklistKey` parameter.
...
name_backbone("Calopteryx splendens", checklistKey = NULL)$usageKey
# 4DXXM

occ_download(
  pred("taxonKey", "4DXXM"),  
  checklistKey = NULL,  # Use GBIF Backbone
  format = "SIMPLE_CSV"
)
```
