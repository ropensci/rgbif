...
Here is example that retrieves the number of species published to GBIF grouped by year and basis of record. Keep in mind that `year` in this context is the collection/event date not the publication date to GBIF.
...
Making a global map of unique species counts per grid cell is a common task, but because it requires a spatial join with the chosen spatial grid, it can be difficult to do without working with sometimes billions of occurrences.
...
path <- "C:/Users/ftw712/Desktop/MGRS/mgrs-grid/mgrs-grid/"
...
In general the hardest part of working with GBIF's sql grid functions is finding the corresponding shapefile or geojson file that matches the grid function used.

EEA reference grid example can be found here:
...
d <- occ_download_get('0071101-240506114902167') %>%
    occ_download_import() %>% 
    filter(!cellcode == "") 

eea_grid <- st_read("C:/Users/ftw712/Desktop/eea_v_3035_10_km_eea-ref-grid-gb_p_2013_v02_r00") %>%
