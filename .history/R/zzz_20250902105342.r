...
gbifparser <- function(input, fields= "minimal") {
  parse <- function(x){
    ...
    # classifications
    # x[names(x) %in% "classifications"] <- list(classifications = tibble::as_tibble(x$classifications))
    
    classifications <- lapply(x$classifications, function(x) { 
      usage <- tibble::as_tibble(x$usage)
      usage$usageCategory <- "USAGE"
      acceptedUsage <- tibble::as_tibble(x$acceptedUsage)  
      acceptedUsage$usageCategory <- "ACCEPTED"
      taxonomicStatus <- x$taxonomicStatus
      
      c <- x$classification
      c <- bind_rows(lapply(x$classification, tibble::as_tibble))
      nv <- stats::setNames(c$name, tolower(c$rank))
      kv <- stats::setNames(c$key,  paste0(tolower(c$rank), "Key"))
      classification <- tibble::as_tibble(as.list(c(nv, kv)))
      classification <- do.call(rbind, replicate(2, classification, simplify = FALSE))
      cc <- rbind(usage, acceptedUsage)
      cc <- cbind(cc, classification)
      cc
    }
    )
    classifications <- lapply(names(classifications), function(x) {
      transform(classifications[[x]], checklistKey = x)
    })

    names(classifications)
    # lapply(classifications, function(x) {
      # names(x)
    # })
