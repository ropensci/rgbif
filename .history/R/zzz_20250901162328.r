...
gbifparser <- function(input, fields= "minimal") {
  parse <- function(x){
    ...
    classifications <- bind_rows(lapply(x$classifications, function(x) { 
      usage <- tibble::as_tibble(x$usage)
      usage$usageCategory <- "USAGE"
      # usage$checklistKey <- names(x$usage)[1]
      ...
      cbind(cc,classification)
    }
    ))
    print(classifications)
    

    # print(names(x) %in% "classifications")
