...
# testthat::test_file("tests/testthat/test-dataset_suggest.R")
...
test_that("dataset_suggest filters by installationKey, endpointType, recordCount, modifiedDate, createdDate, contactEmail", {
...
test_that("dataset_suggest taxonKey shows deprecation warning", {
  skip_on_cran()

    expect_warning(
      dataset_suggest(taxonKey = 212, limit = 1),
      "taxonKey parameter in dataset_suggest\\(\\) only works with the out-of-date GBIF Backbone Taxonomy"
    )
})

test_that("dataset_suggest continent shows deprecation warning", {
  skip_on_cran()
  
  expect_warning(
    dataset_suggest(continent = "EUROPE", limit = 1),
    "continent parameter in dataset_suggest\\(\\) is deprecated"
  )
})
