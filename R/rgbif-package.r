...
#'   \item Is accessible from the `master` branch of the Github repo 
#'   \url{https://github.com/ropensci/rgbif}, check it out there if you want that. 
#' }
#'   
#' The new GBIF API
#' 
#' \itemize{
#'   \item See documentation here: \url{http://dev.gbif.org/wiki/display/POR/Webservice+API}
#'   \item Is being developed on the `newapi` branch of this repo.
#' }
#' 
#' The rgbif branch \code{newapi} will have development code to use the new GBIF API, and will be 
#' folded into the master branch at some later point. You can use functions to get data via 
#' the new GBIF API and old GBIF API in the \code{newapi} branch. The functions for the old GBIF 
#' API give deprecation messages, signaling that they are on their way out. A future version 
#' of \code{rgbif} will remove functions for the old API, so do transition to the functions for 
#' the new API soon.
#' 
