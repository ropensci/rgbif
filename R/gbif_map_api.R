...
get_colours <- function(key, colour_breaks = NULL, colour_nbreaks = NULL){
...
#' \code{optional (default c("OBS","SP","OTH"))}
#'
...
#' \code{optional (default c("1990_2000","2000_2010","2010_2020"))}
#'
...
#' case, the function will try to generate a suitable exponential sequence of
#' break values estimated from the total number of species occurrence records
#' available for this key.
#' \code{optional (default <dynamic>)}
#'
...
#' \code{optional (default 0)}
#'
#' @return An object of the class \code{\link[raster]{raster}}. The raster
#' package should be installed and loaded to handle the output of the function.
#' Raster values represent the number of geo-referenced occurrence records in
#' each of the cells. Notice that values indicate integer ranges ("bins") rather
#' than exact values. The bins can be adjusted using the \code{breaks} parameter.
#'
...
map_fetch <- function(
  ...
  if(!'package:raster' %in% search()){
    stop('"raster" package is required for this package, but was not found.',
         call. = FALSE)
  }
  ...
  if(!is.numeric(key) & !is.integer(key)){
    stop('Value supplied for argument "key" is not numeric.',
         call. = FALSE)
  }
  ...
  temp <- tempfile()
  raw_raster <- GET(url = 'http://api.gbif.org', #gbif_base(),
      path = map_api_path,
      query = query,
  ...
  crs(data_raster) <- '+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0'
